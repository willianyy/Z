...
namespace Z.Utilities.Base.Document.Excel
{
    public class NPOIHelper
    {
        ...
        /// <param name="dtSource">源DataTable</param>
        /// <param name="strHeaderText">表头文本</param>
        public static MemoryStream Export(DataTable dtSource, List<string> strHeaderText)
        {
            HSSFWorkbook workbook = new HSSFWorkbook();
            HSSFSheet sheet = workbook.CreateSheet() as HSSFSheet;
                ...
                if (rowIndex == 65535 || rowIndex == 0)
                {
                    ...
                    #region 表头及样式
                    {
                        if (flag == 0)
                        {
                            HSSFRow headerRow = sheet.CreateRow(0) as HSSFRow;
                            headerRow.HeightInPoints = 25;
                            for (var i = 0; i < strHeaderText.Count; i++)
                            {
                                headerRow.CreateCell(i).SetCellValue(strHeaderText[i]);
                                //HSSFCellStyle headStyle = workbook.CreateCellStyle() as HSSFCellStyle;
                                //HSSFFont font = workbook.CreateFont() as HSSFFont;
                                //font.FontHeightInPoints = 10;
                                //font.Boldweight = 300;
