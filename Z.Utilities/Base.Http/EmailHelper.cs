...
namespace Z.Utilities.Base.Http
{
    public class EmailHelper
    {
        ...
        public EmailHelper(string server, string toMail, string fromMail, string subject, string emailBody, string username, string password, string port, bool sslEnable, bool pwdCheckEnable)
        {
            try
            {
                ...
                if (toMail.Split(new char[]{';'}, StringSplitOptions.RemoveEmptyEntries).Length > 1)
                {
                    foreach (var item in toMail.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries))
                    {
