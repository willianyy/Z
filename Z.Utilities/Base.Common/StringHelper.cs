...
namespace Z.Utilities
{
    ...
    public class StringHelper
    {
        ...
        {
            //md5加密
            byte[] bytes = Encoding.Default.GetBytes(s);    //tbPass为输入密码的文本框
        ...
        /// <param name="content">待处理的字符串</param>
        /// <returns>处理后的字符串</returns>
        public static string RemoveHtmlBySubstring(string content, int len)
        {
            if (string.IsNullOrEmpty(content)) return string.Empty;
            content = RemoveHtml(content);
            content = ClipString(content,len);
            return content;
        }
        ...
        /// </summary>
        /// <param name="str">源字符串</param>
        /// <param name="splitLength">分隔字符数 小于等于0则不分隔返回</param>
        /// <param name="endStr">附加字符串 可为空</param>
        /// <returns></returns>
        public static List<string> GetStrArrayByLength(string str, int splitLength, string endStr = "")
        {
            List<string> list = new List<string>();
            if (string.IsNullOrEmpty(str))
            {
            ...
            if (str.Length <= splitLength || splitLength <= 0)
            {
                list.Add(str);
            }
            else
            {
                ...
                var maxCount = Math.Floor((decimal) (str.Length/splitLength));
                for (var i = 0; i <= maxCount; i++)
                {
                    item = i == maxCount ? str.Substring(i*splitLength) : str.Substring(i*splitLength, splitLength);
                    if (item.Trim().Length==0) continue;
                    list.Add(item + endStr);
                }
            }
        ...
        private static void ParseUrl(string url, out string baseUrl, out System.Collections.Specialized.NameValueCollection nvc)
        {
            if (url == null)
